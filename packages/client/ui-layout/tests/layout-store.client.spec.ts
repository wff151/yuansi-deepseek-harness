...
import {
  ...
  DOCS_DEFAULT, DOCS_MAX, DOCS_MIN,
...
describe('createLayoutStore', () => {
  it('initializes the sidebar at its default width, details closed, wide viewport assumed', () => {
    ...
    expect(store.getSnapshot()).toEqual({ sidebar: SIDEBAR_DEFAULT, details: 0, docs: 0, narrow: false, narrowExpanded: false })
  })
  ...
  it('setSidebar/setDetails/setDocs clamp into the contract ranges', () => {
    ...
    actions.setDocs(1)
    expect(store.getSnapshot().docs).toBe(DOCS_MIN)
    actions.setDocs(9999)
    expect(store.getSnapshot().docs).toBe(DOCS_MAX)
  })
  ...
  it('narrow toggleSidebar flips only the re-expand override; the width preference survives', () => {
    ...
    actions.toggleSidebar()
    expect(store.getSnapshot()).toEqual({ sidebar: 400, details: 0, docs: 0, narrow: true, narrowExpanded: true })
    actions.toggleSidebar()
  ...
  it('openDocs uses the contract default, preserves an open width, and closeDocs zeroes', () => {
    const { store, actions } = createLayoutStore().create()
    actions.openDocs()
    expect(store.getSnapshot().docs).toBe(DOCS_DEFAULT)
    actions.setDocs(500)
    actions.openDocs()
    expect(store.getSnapshot().docs).toBe(500)
    actions.closeDocs()
    expect(store.getSnapshot().docs).toBe(0)
  })
